...
class NativeDecoder
{
    ...
    private:
        ...
        void orientDecodedTile(uint32_t, uint32_t, uint32_t *, uint32_t *);
        void flipPixelsVertical(uint32_t, uint32_t, jint *);
        void flipPixelsHorizontal(uint32_t, uint32_t, jint *);
        jint *getSampledRasterFromTile(int, int *, int *);
        jint *getSampledRasterFromTileWithBounds(int, int *, int *);
        int getDecodeMethod();
        void fixOrientation(jint *, uint32_t, int, int);
